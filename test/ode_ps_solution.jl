...
@testset "Power series solution for an ODE system" begin
    ...
    R, (x, y, x_dot, y_dot, u) = Nemo.PolynomialRing(Nemo.QQ,
                                                     ["x", "y", "x_dot", "y_dot", "u"])
    prec = 100
    eqs = [
        x_dot - x + 3 * x * y - u,
    ...
    @test map(e -> valuation(evaluate(e, [sol[v] for v in gens(R)])), eqs) ==
          [prec - 1, prec - 1]
    ...
    for i in 1:30
        ...
        inputs = Dict(u => [F(rand(-3:3)) for i in 1:prec]
                      for u in vars[(2 * NUMX + 1):end])
        ...
        eqs = [denominators[i] * vars[i] - rand_poly(2, vars[(NUMX + 1):end])
               for i in 1:NUMX]
