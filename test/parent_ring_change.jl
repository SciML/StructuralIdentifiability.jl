@testset "Parent ring change" begin
    for field in [Nemo.QQ, Nemo.GF(2^31 - 1)]
        ...
        f = 2x + 3y + x^7 * y
        @test f == parent_ring_change(f, R, matching = :byname)
        @test f == parent_ring_change(f, R, matching = :byindex)

        f_ = parent_ring_change(f, R_, matching = :byname)
        f__ = parent_ring_change(f, R__, matching = :byname)
        @test f_ == 2x_ + 3y_ + x_^7 * y_
        @test f__ == 2x__ + 3y__ + x__^7 * y__
        @test f == parent_ring_change(f_, R, matching = :byname)
        @test f == parent_ring_change(f__, R, matching = :byname)
        ...
        @test f == 2z + 5x^3 + 3y^2
        @test f__ == parent_ring_change(f, R__, matching = :byindex)
    end
end
