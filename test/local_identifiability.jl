@safetestset "Assessing local identifiability" begin
    ...
    if Sys.WORD_SIZE != 64
        @info "Skipping local_identifiability.jl on $(Sys.WORD_SIZE)-bit (Nemo#2358)"
    else
        include(joinpath(@__DIR__, "bodies", "local_identifiability.jl"))
    end
end
