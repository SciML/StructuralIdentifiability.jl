@testset "Computing variations around a sequence solution" begin
    ...
    function use_lie_derivatives(
        ...
        append!(newvars, [var_to_str(v) * "$i" for v in dds.u_vars for i in 1:num_terms])
        R, _ = StructuralIdentifiability.Nemo.PolynomialRing(base_ring(dds.poly_ring), newvars)
        explicit_sol = merge(
            Dict(parent_ring_change(x, R) => Vector{Any}([parent_ring_change(x, R)]) for (x, eq) in dds.x_equations),
            Dict(parent_ring_change(y, R) => Vector{Any}([parent_ring_change(eq, R)]) for (y, eq) in dds.y_equations)
        )
        time_step = merge(
            Dict(parent_ring_change(x, R) => parent_ring_change(eq, R) for (x, eq) in dds.x_equations),
            Dict(parent_ring_change(p, R) => parent_ring_change(p, R) for p in dds.parameters),
            Dict(parent_ring_change(u, R) => str_to_var(var_to_str(u) * "1", R) for u in dds.u_vars),
            Dict(str_to_var(s * "$i", R) => str_to_var(s * "$(i + 1)", R) for s in map(var_to_str, dds.u_vars) for i in 1:(num_terms - 1))
        )
        eval_dict = merge(
            ...
            Dict(str_to_var(var_to_str(u) * "$i", R) => inputs[u][i + 1] for u in dds.u_vars for i in 1:(num_terms - 1))
        )
        generalized_parameters = [parent_ring_change(p, R) for p in vcat(dds.x_vars, dds.parameters)]
        for i in 2:num_terms
            for (k, v) in explicit_sol
                push!(explicit_sol[k], eval_at_dict(v[end], time_step))
            end
        end
        part_diffs = Dict((f, p) => [] for f in keys(explicit_sol) for p in generalized_parameters)
        ...
        return Dict(
            (parent_ring_change(k[1], dds.poly_ring), parent_ring_change(k[2], dds.poly_ring)) => res
            for (k, res) in part_diffs
        )
    end
