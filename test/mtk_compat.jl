@testset "Check identifiability of `ODESystem` object" begin
    ...
    correct = Dict(a12 => :nonidentifiable, a01 + a12 + a21 => :globally, a01 * a12 => :globally,
                   a21 => :nonidentifiable, a01 => :nonidentifiable)
    ...
    correct = Dict(a12 => :nonidentifiable, a01 + a12 + a21 => :globally, a01 * a12 => :globally,
                   a21 => :nonidentifiable, a01 => :nonidentifiable)
    @test isequal(correct,
                  assess_identifiability(de; measured_quantities = measured_quantities,
    ...
    eqs = [D(S) ~ mu - bi * S * I - bw * S * W - mu * S + a * R,
    ...
    @test isequal(true,
                  all(values(assess_local_identifiability(de;
    ...
    correct = Dict(f => true for f in funcs_to_check)
    @test isequal((correct, 1),
                  assess_local_identifiability(de; measured_quantities = measured_quantities,
                                               funcs_to_check = funcs_to_check, p = 0.99,
                                               type = :ME))
    ...
    correct = Dict(f => true for f in funcs_to_check)
    @test isequal((correct, 1),
                  assess_local_identifiability(de; funcs_to_check = funcs_to_check, p = 0.99,
                                               type = :ME))
    ...
    eqs = [D(S) ~ 2.0 * mu - bi * S * I - bw * S * W - mu * S + a * R,
           D(I) ~ bw * S * W + bi * S * I - (gm + mu) * I,
           D(W) ~ xi * (I - 0.6 * W),
           D(R) ~ gm * I - (mu + a) * R]
    de = ODESystem(eqs, t; name = :TestSIWR)
    ...
    eqs = [D(x0) ~ -(a01 + a21) * SpecialFunctions.erfc(x0) + a12 * x1,
           D(x1) ~ a21 * x0 - a12 * x1]
    ...
    funcs_to_check = [a01, a21, a12, a01 * a12, a01 + a12 + a21]
    correct = Dict(a01 => :nonidentifiable, a21 => :nonidentifiable, a12 => :nonidentifiable,
                   a01 * a12 => :globally, a01 + a12 + a21 => :globally)
    @test_throws ArgumentError assess_identifiability(de; measured_quantities = measured_quantities,
                                                      funcs_to_check = funcs_to_check)
    # ----------
    @parameters a b c
    ...
    eqs = [D(x1) ~ -a * x1 + x2 * b / (x1 + b / (c^2 - x2)),
           D(x2) ~ x2 * c^2 + x1]
    de = ODESystem(eqs, t; name = :Test)
