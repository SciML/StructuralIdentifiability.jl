...
function diff_sol_Lie_derivatives(ode::ODE, params, ic, inputs, prec::Int)
    ...
    params, ic = map(d -> Dict(str_to_var(string(k), new_ring) => v for (k, v) in d),
                     [params, ic])
    ...
    for u in ode.u_vars
        for i in 0:(prec - 2)
            derivation[str_to_var(var_to_str(u) * "_$i", new_ring)] = str_to_var(var_to_str(u) *
                                                                                 "_$(i + 1)",
                                                                                 new_ring)
        end
    end
    ...
    for (y, g) in ode.y_equations
        ...
        for i in 1:prec
            push!(Lie_derivatives[y],
                  sum(derivative(Lie_derivatives[y][end], v) * get(derivation, v, 0)
                      for v in gens(new_ring)))
        end
    end
    ...
    for u in ode.u_vars
        for i in 1:prec
            eval_point[str_to_var(var_to_str(u) * "_$(i - 1)", new_ring)] = inputs[u][i] *
                                                                            factorial(i - 1)
        end
    end
    ...
    for y in ode.y_vars
        ...
        for v in vcat(ode.x_vars, ode.parameters)
            ...
            for j in 1:prec
                push!(result[y][v],
                      eval_at_dict(derivative(Lie_derivatives[y][j],
                                              str_to_var("$v", new_ring)), eval_point))
            end
        end
    end
...
@testset "Partial derivatives of an output w.r.t. to initial conditions and parameters" begin
    ...
    ode = @ODEmodel(x'(t)=x(t)^2 + 2 * x(t) * y(t) - 3 * a * y(t),
    ...
    varnames = vcat(["x_$i" for i in 1:3],
                    ["p_$i" for i in 1:3],
                    ["u_$i" for i in 1:2],
                    ["y_$i" for i in 1:3])
    R, vars = Nemo.PolynomialRing(F, varnames)
    push!(test_cases,
          Dict(:ODE => ODE{P}(Dict{P, DType}(vars[i] => rand_poly(1, vars[1:8])
                                             for i in 1:3),
                              Dict{P, DType}(vars[i] => rand_poly(2, vars[1:8])
                                             for i in 9:11),
                              vars[7:8]),
    ...
    R, vars = Nemo.PolynomialRing(F, varnames)
    push!(test_cases,
          Dict(:ODE => ODE{P}(Dict{P, DType}(vars[i] => rand_poly(2, vars[1:8])
                                             for i in 1:3),
                              Dict{P, DType}(vars[i] => rand_poly(2, vars[1:8])
                                             for i in 9:11),
                              vars[7:8]),
    ...
    varnames = vcat(["x_$i" for i in 1:2],
                    ["p_$i" for i in 1:2],
                    "u",
                    ["y_1", "y_2"])
    R, vars = Nemo.PolynomialRing(F, varnames)
    push!(test_cases,
          Dict(:ODE => ODE{P}(Dict{P, DType}(vars[i] => rand_poly(1, vars[1:5]) //
                                                        (vars[1] + vars[3]) for i in 1:2),
                              Dict{P, DType}(vars[i] => rand_poly(1, vars[1:5])
                                             for i in 6:7),
                              [vars[5]]),
    ...
    for case in test_cases
        ...
        @time sol1 = differentiate_output(ode, case[:param_vals], case[:ic], case[:inputs],
                                          prec)
        sol2 = diff_sol_Lie_derivatives(ode, case[:param_vals], case[:ic], case[:inputs],
                                        prec)
        for y in ode.y_vars
            for v in vcat(ode.x_vars, ode.parameters)
                @test sol2[y][v] ==
                      [base_ring(ode.poly_ring)(coeff(sol1[y][v], j) * factorial(j))
                       for j in 0:(prec - 1)]
            end
        end
    end
