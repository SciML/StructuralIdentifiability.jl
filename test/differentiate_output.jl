...
function diff_sol_Lie_derivatives(ode::ODE, params, ic, inputs, prec::Int)
    ...
    if length(ode.u_vars) > 0
        append!(new_varnames, [var_to_str(u) * "_$i" for u in ode.u_vars for i in 0:(prec - 1)])
    end
    ...
    for y in ode.y_vars
        ...
        for v in vcat(ode.x_vars, ode.parameters)
            ...
            for j in 1:prec
                push!(result[y][v],
                      eval_at_dict(derivative(Lie_derivatives[y][j], str_to_var("$v", new_ring)),
                                   eval_point))
            end
        end
    end
...
@testset "Partial derivatives of an output w.r.t. to initial conditions and parameters" begin
    ...
    push!(test_cases,
          Dict(:ODE => ODE{P}(Dict{P,DType}(vars[i] => rand_poly(1, vars[1:8]) for i in 1:3),
    ...
    R, vars = Nemo.PolynomialRing(F, varnames)
    push!(test_cases,
          Dict(:ODE => ODE{P}(Dict{P,DType}(vars[i] => rand_poly(2, vars[1:8]) for i in 1:3),
                              Dict{P,DType}(vars[i] => rand_poly(2, vars[1:8]) for i in 9:11),
                              vars[7:8]),
               :ic => Dict(vars[i] => F(rand(1:50)) for i in 1:3),
               :param_vals => Dict(vars[i + 3] => F(rand(1:50)) for i in 1:3),
    ...
    for case in test_cases
        ...
        @time sol1 = differentiate_output(ode, case[:param_vals], case[:ic], case[:inputs], prec)
        sol2 = diff_sol_Lie_derivatives(ode, case[:param_vals], case[:ic], case[:inputs], prec)
        for y in ode.y_vars
            for v in vcat(ode.x_vars, ode.parameters)
                @test sol2[y][v] == [base_ring(ode.poly_ring)(coeff(sol1[y][v], j) * factorial(j))
                                     for j in 0:(prec - 1)]
            end
        end
    end
