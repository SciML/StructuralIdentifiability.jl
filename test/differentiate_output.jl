...
function diff_sol_Lie_derivatives(ode::ODE, params, ic, inputs, prec::Int)
    ...
    for u in ode.u_vars
        for i in 0:(prec - 2)
            derivation[str_to_var(var_to_str(u) * "_$i", new_ring)] = str_to_var(var_to_str(u) * "_$(i + 1)", new_ring)
        end
    end
    ...
    for u in ode.u_vars
        for i in 1:prec
            eval_point[str_to_var(var_to_str(u) * "_$(i - 1)", new_ring)] = inputs[u][i] * factorial(i - 1)
        end
    end
    ...
    for y in ode.y_vars
        ...
        for v in vcat(ode.x_vars, ode.parameters)
            ...
            for j in 1:prec
                push!(
                    result[y][v], 
                    eval_at_dict(derivative(Lie_derivatives[y][j], str_to_var("$v", new_ring)), eval_point)
                )
            end
        end
    end
...
@testset "Partial derivatives of an output w.r.t. to initial conditions and parameters" begin
    ...
    push!(
        ...
        Dict(
          :ODE => ODE{P}(
              Dict{P, DType}(vars[i] => rand_poly(1, vars[1:5]) // (vars[1] + vars[3]) for i in 1:2), 
    ...
    for case in test_cases
        ...
        for y in ode.y_vars
            for v in vcat(ode.x_vars, ode.parameters)
                @test sol2[y][v] == [base_ring(ode.poly_ring)(coeff(sol1[y][v], j) * factorial(j)) for j in 0:(prec - 1)]
            end
        end
    end
