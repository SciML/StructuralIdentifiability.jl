...
fracs_generators = [a^2 + b^2, a^3 + b^3, a^4 + b^4]
to_be_reduced = [a^6 + b^6, a^7 + b^7, a^8 + b^8]
...
function reparametrize_with_respect_to(ode, new_states, new_params)
    ...
    if !isempty(inputs)
        ...
        new_inputs = Dict(
            input => new_input for (input, new_input) in zip(inputs, new_inputs_dynamics)
        )
    else
        new_inputs = Dict()
    end
...
function reparametrize_global(ode)
    id_funcs = StructuralIdentifiability.find_identifiable_functions(
        ode,
        with_states = true,
        strategy = (:hybrid,),
