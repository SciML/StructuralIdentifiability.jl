...
function generator_var_change(generator, var::P, numer::P, denom::P) where {P <: MPolyElem}
    return IterTools.imap(point -> begin
                              result = copy(point)
                              result[var] = eval_at_dict(numer, point) //
                                            eval_at_dict(denom, point)
                              return result
                          end,
                          Iterators.filter(point -> eval_at_dict(denom, point) != 0,
                                           generator))
end
...
function generate_io_equation_problem(ode::ODE{P}) where {P <: MPolyElem{<:FieldElem}}
    ...
    for i in 0:(dim_x - 1)
        for y in ode.y_vars
            derivation[str_to_var(var_to_str(y) * "_$i", ring)] = str_to_var(var_to_str(y) *
                                                                             "_$(i + 1)",
                                                                             ring)
        end
    end
    for u in ode.u_vars
        for i in 0:(dim_x - 1)
            derivation[str_to_var(var_to_str(u) * "_$i", ring)] = str_to_var(var_to_str(u) *
                                                                             "_$(i + 1)",
                                                                             ring)
        end
    end
...
"""
function find_ioequations(ode::ODE{P};
                          var_change_policy = :default) where {P <: MPolyElem{<:FieldElem}}
    ...
    while true
        var_degs = [(y,
                     [Nemo.degree(eq, x)
                      for x in keys(x_equations) if Nemo.degree(eq, x) > 0])
        ...
        outputs_with_scores = [(min(d[2]...) * length(y_equations[d[1]]),
                                min(d[2]...),
                                -count(x -> x == min(d[2]...), d[2]) +
                                length(y_equations[d[1]]) // 30,
                                length(y_equations[d[1]]),
                                d[1]) for d in var_degs]
        ...
        for (x, eq) in x_equations
            ...
            flush(stdout)
            next_y_equation = eliminate_var(eq, next_y_equation, derivation[x],
                                            point_generator)
        end
        for (y, eq) in y_equations
            if y != y_prolong
                ...
                next_y_equation = eliminate_var(next_y_equation, eq,
                                                str_to_var(var_to_str(y)[1:(end - 2)] *
                                                           "_$(y_orders[y])", ring),
                                                point_generator)
            end
        end
        ...
        var_degs_next = [(Nemo.degree(y_equations[y_prolong], x),
                          Nemo.degree(next_y_equation, x), x)
                         for x in keys(x_equations)
                         if Nemo.degree(y_equations[y_prolong], x) > 0]
            ...
            for x in setdiff(keys(x_equations), [var_elim])
                if Nemo.degree(Ay_plus_B, x) == 1
                    ...
                    if isempty(filter!(v -> (v in keys(x_equations)), vars(A))) && (B != 0) # && (length(coeffs(A))==1)
                        ...
                        point_generator = generator_var_change(point_generator,
                                                               derivation[x],
                                                               derivation[x] * denom_d +
                                                               numer_d, denom_d)
                        point_generator = generator_var_change(point_generator, x,
                                                               A * x + B, A)

                        # Change current system
                        @debug "Change in the system"
                        flush(stdout)
                        x_equations[x] = make_substitution(x_equations[x], derivation[x],
                                                           denom_d * derivation[x] -
                                                           numer_d, denom_d)
                        for xx in keys(x_equations)
                            @debug "\t Change in the equation for $xx"
                            flush(stdout)
                            x_equations[xx] = make_substitution(x_equations[xx], x,
                                                                A * x - B, A)
                        end
                        ...
                        for y in keys(y_equations)
                            ...
                            flush(stdout)
                            y_equations[y] = make_substitution(y_equations[y], x, A * x - B,
                                                               A)
                        end
                        @debug "\t Change in the prolonged equation"
                        flush(stdout)
                        next_y_equation = make_substitution(next_y_equation, x, A * x - B,
                                                            A)
                        # recalibrate system
                        ...
                        for xx in setdiff(keys(x_equations), [x])
                            @debug "\t Unmixing $xx"
                            flush(stdout)
                            x_equations[x] = eliminate_var(x_equations[x], x_equations[xx],
                                                           derivation[xx], point_generator)
                        end
        ...
        for (x, eq) in x_equations
            ...
            flush(stdout)
            x_equations[x] = eliminate_var(eq, y_equations[y_prolong], var_elim,
                                           point_generator)
        end
        ...
        for y in keys(y_equations)
            if y != y_prolong
                ...
                flush(stdout)
                y_equations[y] = eliminate_var(y_equations[y], y_equations[y_prolong],
                                               var_elim, point_generator)
            end
        end
        @debug "\t Elimination in the prolonged equation"
        flush(stdout)
        y_equations[y_prolong] = eliminate_var(y_equations[y_prolong], next_y_equation,
                                               var_elim, point_generator)
    end

    io_equations = Dict(str_to_var(var_to_str(y)[1:(end - 2)] * "_$(y_orders[y])", ring) => p
                        for (y, p) in y_equations)
    ...
    while true
        ...
        point_generator = generator_var_change(point_generator, extra_var, extra_var_rhs,
                                               one(ring))
