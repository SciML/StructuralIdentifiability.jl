...
"""
@timeit _to function beautifully_generators(
...
@timeit _to function generating_sets_fan(
    ...
    cfs = beautifully_generators(new_rff)
    ...
    if code >= 1
        for i in 1:nbases
            ...
            cfs = beautifully_generators(new_rff, discard_redundant = false)
            ordering_to_generators[ord] = cfs
        end
    end
    if code >= 2
...
@timeit _to function simplified_generating_set(
    ...
    new_fracs = beautifully_generators(new_rff)
...
Out of $(length(new_fracs)) fractions $(length(new_fracs_unique)) are syntactically unique."""
    runtime =
        @elapsed new_fracs = beautifully_generators(RationalFunctionField(new_fracs_unique))
