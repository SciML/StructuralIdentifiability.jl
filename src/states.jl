...
function extract_coefficients_ratfunc(f::P, vars::Vector{<: P}) where {P <: MPolyElem{<:FieldElem}}
...
Output:
- a list of rational functions in paarmeters and states genrating the field of all identifiabile functions over
the field generated by the inputs and the identifiable functions of parameters only
"""
function states_generators(ode::ODE{P}, io_equations::Dict{P, P}) where {P <: MPolyElem{<: FieldElem}}
    y_to_ord = Dict{P, Int}()
    ...
    for (y, ord) in y_to_ord
        ...
        for _ in 0:ord
            curr = filter(x -> total_degree(numerator(x)) + total_degree(denominator(x)) > 0, curr)
            append!(result, curr)
            curr = vcat([extract_coefficients_ratfunc(lie_derivative(f, ode), ode.u_vars) for f in  curr]...)
        end
    end
