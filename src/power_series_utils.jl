...
function _matrix_inv_newton_iteration(M::MatElem{T},
                                      Minv::MatElem{T}) where {T<:Generic.AbsSeriesElem{<:Generic.FieldElem}}
...
function _matrix_homlinear_de_newton_iteration(A::MatElem{<:Generic.AbsSeriesElem{T}},
    ...
    Y = Y - truncate_matrix(Y *
                            map(ps_integrate, Z * (Yprime - truncate_matrix(A, 2 * cur_prec - 1) * Y)),
                            2 * cur_prec)
    return (Y, Z)
end
...
function ps_ode_solution(equations::Array{P,1},
    ...
    return ps_ode_solution(equations,
