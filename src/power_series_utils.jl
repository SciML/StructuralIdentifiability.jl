...
"""
function _matrix_inv_newton_iteration(M::MatElem{T},
                                      Minv::MatElem{T}) where {
                                                               T <: Generic.AbsSeriesElem{
                                                                                     <:Generic.FieldElem
                                                                                     }}
...
"""
function ps_matrix_inv(M::MatElem{<:Generic.AbsSeriesElem{<:Generic.FieldElem}},
                       prec::Int = -1)
...
function _matrix_homlinear_de_newton_iteration(A::MatElem{<:Generic.AbsSeriesElem{T}},
                                               Y::MatElem{<:Generic.AbsSeriesElem{T}},
                                               Z::MatElem{<:Generic.AbsSeriesElem{T}},
                                               cur_prec::Int) where {T <: Generic.FieldElem}
    Yprime = map(ps_diff, Y)
    Z = Z + truncate_matrix(Z * (one(parent(A)) - Y * Z), cur_prec)
    Y = Y - truncate_matrix(Y * map(ps_integrate,
                            Z * (Yprime - truncate_matrix(A, 2 * cur_prec - 1) * Y)),
                        2 * cur_prec)
    return (Y, Z)
end
...
"""
function ps_matrix_homlinear_de(A::MatElem{<:Generic.AbsSeriesElem{T}},
                                Y0::MatElem{<:T},
                                prec::Int = -1) where {T <: Generic.FieldElem}
    prec = (prec == -1) ? precision(A[1, 1]) : prec
    ps_ring = base_ring(parent(A))
    cur_prec = 1
    Y = (one(parent(A)) + gen(ps_ring) * truncate_matrix(A, cur_prec)) *
...
"""
function ps_matrix_linear_de(A::MatElem{<:Generic.AbsSeriesElem{T}},
                             B::MatElem{<:Generic.AbsSeriesElem{T}},
                             Y0::MatElem{<:T},
                             prec::Int = -1) where {T <: Generic.FieldElem}
    prec = (prec == -1) ? precision(A[1, 1]) : prec
...
"""
function ps_ode_solution(equations::Array{P, 1},
                         ic::Dict{P, T},
...
function ps_ode_solution(equations::Array{P, 1},
                         ...
                         prec::Int) where {P <: MPolyElem{<:Generic.FieldElem}}
    bring = base_ring(parent(equations[1]))
    ps_ode_solution(equations,
