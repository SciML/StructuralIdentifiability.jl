...
function check_identifiability(
    ...
    funcs_to_check = 
        Vector{Generic.Frac{P}}(map(f -> parent_ring_change(f, bring) // one(bring), funcs_to_check))

    return field_contains(RationalFunctionField(identifiable_functions_raw), funcs_to_check, p)
end
