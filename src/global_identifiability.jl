...
function check_field_membership(generators::Array{<:Array{<:MPolyElem,1},1},
    ...
    sampling_bound = BigInt(3 * BigInt(degree)^(length(total_vars) + 3) * length(rat_funcs) *
                            ceil(1 / (1 - p)))
    # sampling_bound = 5
...
function check_identifiability(io_equations::Array{P,1}, parameters::Array{P,1}, p::Float64 = 0.99;
                               method = :GroebnerBasis) where {P<:MPolyElem{fmpq}}
    return check_identifiability(io_equations, parameters, parameters, p; method = method)
end
...
function assess_global_identifiability(ode::ODE{P},
                                       p::Float64 = 0.99;
                                       var_change = :default,
                                       gb_method = :GroebnerBasis) where {P<:MPolyElem{fmpq}}
...
function simplify_field_generators(generators::Array{<:Array{<:MPolyElem,1},1})
    ...
    total_vars = foldl(union,
                       map(plist -> foldl(union,
                                          map(poly -> Set(map(var_to_str, vars(poly))), plist)),
                           generators))
...
function extract_identifiable_functions(io_equations::Array{P,1},
                                        parameters::Array{P,1},
                                        known_functions::Array{P,1}) where {P<:MPolyElem{fmpq}}
    @debug "Extracting coefficients"
    flush(stdout)
    nonparameters = filter(v -> !(var_to_str(v) in map(var_to_str, parameters)),
                           gens(parent(io_equations[1])))
...
function extract_identifiable_functions_raw(io_equations::Array{P,1},
                                            parameters::Array{P,1}) where {P<:MPolyElem{fmpq}}
    @debug "Extracting coefficients"
    flush(stdout)
    nonparameters = filter(v -> !(var_to_str(v) in map(var_to_str, parameters)),
                           gens(parent(io_equations[1])))
