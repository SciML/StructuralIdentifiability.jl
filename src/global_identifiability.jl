...
"""
function check_field_membership(generators::Array{<:Array{<:MPolyElem, 1}, 1},
    ...
    for f in rat_funcs
        ...
        degree = max(degree,
                     total_degree(total_lcm) - total_degree(den) + total_degree(num))
    end
    @debug "\tBound for the degrees is $degree"
    total_vars = foldl(union,
                       map(plist -> foldl(union, map(poly -> Set(vars(poly)), plist)),
                           generators))
    @debug "\tThe total number of variables in $(length(total_vars))"

    sampling_bound = BigInt(3 * BigInt(degree)^(length(total_vars) + 3) *
                            length(rat_funcs) * ceil(1 / (1 - p)))
    # sampling_bound = 5
    ...
    ring_sing, vars_sing = Singular.PolynomialRing(Singular.QQ,
                                                   vcat(map(var_to_str, gens(ring)),
                                                        ["sat_aux$i"
...
function check_identifiability(io_equations::Array{P, 1},
                               parameters::Array{P, 1},
                               funcs_to_check::Array{<:Any, 1},
                               p::Float64 = 0.99;
                               method = :GroebnerBasis) where {P <: MPolyElem{fmpq}}
    @debug "Extracting coefficients"
    flush(stdout)
    nonparameters = filter(v -> !(var_to_str(v) in map(var_to_str, parameters)),
...
function check_identifiability(io_equation::P,
                               parameters::Array{P, 1},
                               funcs_to_check::Array{<:Any, 1},
                               p::Float64 = 0.99;
                               method = :GroebnerBasis) where {P <: MPolyElem{fmpq}}
...
function check_identifiability(io_equations::Array{P, 1}, parameters::Array{P, 1},
                               p::Float64 = 0.99;
                               method = :GroebnerBasis) where {P <: MPolyElem{fmpq}}
    check_identifiability(io_equations, parameters, parameters, p; method = method)
end

function check_identifiability(io_equation::P, parameters::Array{P, 1}, p::Float64 = 0.99;
                               method = :GroebnerBasis) where {P <: MPolyElem{fmpq}}
...
"""
function assess_global_identifiability(ode::ODE{P},
                                       p::Float64 = 0.99;
                                       var_change = :default,
                                       gb_method = :GroebnerBasis) where {
                                                                          P <:
                                                                          MPolyElem{fmpq}}
    result_list = assess_global_identifiability(ode, ode.parameters, p;
...
function assess_global_identifiability(ode::ODE{P},
    ...
    ioeq_time = @elapsed io_equations = find_ioequations(ode;
    ...
    check_time = @elapsed result = check_identifiability(collect(values(io_equations)),
                                                         ode.parameters, funcs_to_check, p;
                                                         method = gb_method)
...
function simplify_field_generators(generators::Array{<:Array{<:MPolyElem, 1}, 1})
    ...
    total_vars = foldl(union,
                       map(plist -> foldl(union,
                                          map(poly -> Set(map(var_to_str, vars(poly))),
                                              plist)), generators))
    ...
    ring_sing, vars_sing = Singular.PolynomialRing(F,
                                                   vcat(total_vars,
                                                        ["sat_aux$i"
                                                         for i in 1:length(generators)]);
                                                   ordering = :degrevlex)
...
"""
function extract_identifiable_functions(io_equations::Array{P, 1},
                                        parameters::Array{P, 1},
                                        known_functions::Array{P, 1}) where {
                                                                             P <:
                                                                             MPolyElem{fmpq
                                                                                       }}
    @debug "Extracting coefficients"
...
"""
function extract_identifiable_functions_raw(io_equations::Array{P, 1},
                                            parameters::Array{P, 1}) where {
                                                                            P <:
                                                                            MPolyElem{fmpq}}
    @debug "Extracting coefficients"
    flush(stdout)
    ...
    result = []
    for eq in io_equations
        coeffs = sort(collect(values(extract_coefficients(eq, nonparameters))),
                      by = total_degree)
...
function find_identifiable_functions(ode::ODE{<:MPolyElem{fmpq}}, p::Float64 = 0.99)
    ...
    return extract_identifiable_functions(collect(values(io_equations)), ode.parameters,
                                          known_params)
end
