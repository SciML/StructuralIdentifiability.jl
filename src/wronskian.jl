...
function monomial_compress(io_equation, params::Array{<:MPolyElem, 1})
    other_vars = [v
                  for v in gens(parent(io_equation))
                  if !(var_to_str(v) in map(var_to_str, params))]
