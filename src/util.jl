...
function eval_at_dict(rational::Generic.Frac{<:P},
                      d::Dict{P, <:RingElem}) where {P <: MPolyElem}
    f, g = unpack_fraction(rational)
    return eval_at_dict(f, d) * inv(eval_at_dict(g, d))
end

function eval_at_dict(rational::Generic.Frac{<:P},
                      d::Dict{P, <:Generic.Frac}) where {P <: MPolyElem}
    f, g = unpack_fraction(rational)
    return eval_at_dict(f, d) // eval_at_dict(g, d)
end

function eval_at_dict(rational::Generic.Frac{<:P},
                      d::Dict{P, <:MPolyElem}) where {P <: MPolyElem}
    f, g = unpack_fraction(rational)
    return eval_at_dict(f, d) // eval_at_dict(g, d)
end
...
"""
function make_substitution(f::P, var_sub::P, val_numer::P,
                           val_denom::P) where {P <: MPolyElem}
...
function extract_coefficients(poly::P, variables::Array{P, 1}) where {P <: MPolyElem}
    ...
    coeff_vars = filter(v -> !(var_to_str(v) in map(var_to_str, variables)),
                        gens(parent(poly)))
    new_ring, new_vars = Nemo.PolynomialRing(base_ring(parent(poly)),
                                             map(var_to_str, coeff_vars))
    coeff_var_to_ind = Dict((v, findfirst(e -> (e == v), gens(parent(poly))))
                            for v in coeff_vars)
...
function eval_at_nemo(e::Union{ModelingToolkit.Symbolics.Sym, ModelingToolkit.Symbolics.Term
                               }, vals::Dict)
