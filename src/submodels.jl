...
function dfs(graph::Dict{fmpq_mpoly, Set{fmpq_mpoly}}, start::fmpq_mpoly,
             visited::Set{fmpq_mpoly})
    push!(visited, start)
...
function traverse_outputs(graph::Dict{fmpq_mpoly, Set{fmpq_mpoly}},
                          ys::Array{fmpq_mpoly, 1})
...
function filter_max(ode::ODE{P},
                    submodels::Array{Set{fmpq_mpoly}, 1}) where {P <: MPolyElem}
    n = length(ode.x_vars)
...
function ode_aux(ode::ODE{P}, submodel::Set{fmpq_mpoly}) where {P <: MPolyElem}
    ...
    fin_x = dict_type(parent_ring_change(x, S) => parent_ring_change(f, S)
                      for (x, f) in new_x)
    fin_y = dict_type(parent_ring_change(y, S) => parent_ring_change(f, S)
                      for (y, f) in new_y)
...
function submodel2ode(ode::ODE{P},
                      submodels::Array{Set{fmpq_mpoly}, 1}) where {P <: MPolyElem}
